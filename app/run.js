...
let userProjectsSourceDir = path.join(__dirname, '/../projects')
...
async function readArgs (config) {
  let { configPath, percyToken, port, project_dir} =  config
  process.env["PERCY_TOKEN"] = percyToken?percyToken:process.env.PERCY_TOKEN
  defPdfConfigFilePath =  configPath?configPath:'../configs/pdf-run-info.yml'
  PORT = port?port:8080
  userProjectsSourceDir =  project_dir?project_dir:path.join(__dirname, '/../projects')
...
async function triggerPercyProcess (
  ...
  console.info(
    `Triggering Percy Build | percy snapshot --verbose "${percyConfigFilePath}" with PERCY_BRANCH = ${percyBranch.replace(
      /\s/g,
      ''
